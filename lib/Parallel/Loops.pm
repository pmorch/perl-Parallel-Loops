...
=head1 SYNOPSIS
    ...
    # caculation's result in %output. For that reason, we "tie" %output, so that
    # changes to %output in any child process (see below) are automatically
    # transfered and updated in the parent also.
